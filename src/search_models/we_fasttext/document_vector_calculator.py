...
class DocumentVectorCalculator(Calculator):
    ...
    def create_mean_embedding(self, model, words):
    ...
    def create_max_embedding(self, model, words):
    ...
    def create_document_embedding(self, model, words):
        ...
        """
        return concatenate([self.create_mean_embedding(model, words), self.create_max_embedding(model, words)])


    def calculate_embeddings_for_all_documents(self, trained_model:File, preprocessed_merged_corpus: File, ordered_file_list: list[str]):
        ...
        for idx, line in enumerate(preprocessed_merged_corpus.load(use_iterator=True)):
            ...
            document_embedding = self.create_document_embedding(model, processed_words)
